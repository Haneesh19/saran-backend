...
const MyProfile = ({ darkMode, email }) => {
 ...
 useEffect(() => {
   const fetchProfileStatistics = async () => {
     try {
       const bidsResponse = await axios.get(`https://saran-frontend.onrender.com/api/getTotalBids/${userId}`);
       setTotalBids(bidsResponse.data.totalBids);

       const productsResponse = await axios.get(`https://saran-frontend.onrender.com/api/getTotalProducts/${userId}`);
       setTotalProducts(productsResponse.data.totalProducts);

       const winningBidsResponse = await axios.get(`https://saran-frontend.onrender.com/api/getWinningBids/${userId}`);
